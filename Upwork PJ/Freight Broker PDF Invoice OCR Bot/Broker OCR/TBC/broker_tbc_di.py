...
where 1=1 and us_broker_id = 231 and ship_date >= CURRENT_DATE - INTERVAL '6 months';", host=host, passwd=passwd, name=name, db=db)
...
condition = "invoice_date >= date_trunc('month',(select max(invoice_date) from y4a_erp.y4a_erp_actual_broker where channel = 'DI' and name ='TBC') - interval '1 month') and channel = 'DI' and name ='TBC'"
...
true_data_by_line.rename({'amount':'total_amount'}, inplace = True)

condition2 = "invoice_date >= date_trunc('month',(select max(invoice_date) from y4a_erp.y4a_erp_actual_broker_detail where channel = 'DI' and name ='TBC') - interval '1 month') and channel = 'DI' and name ='TBC'"
