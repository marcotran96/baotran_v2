...
where 1=1 and us_broker_id = 6 and ship_date >= CURRENT_DATE - INTERVAL '6 months';", host=host, passwd=passwd, name=name, db=db)
if win_sys.empty:
...
condition = "invoice_date >= date_trunc('month',(select max(invoice_date) from y4a_erp.y4a_erp_actual_broker where channel = 'DI' and name ='WSL') - interval '1 month') and channel = 'DI' and name ='WSL'"
...
condition2 = "invoice_date >= date_trunc('month',(select max(invoice_date) from y4a_erp.y4a_erp_actual_broker_detail where channel = 'DI' and name ='WSL') - interval '1 month') and channel = 'DI' and name ='WSL'"
